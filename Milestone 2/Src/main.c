...
volatile uint8_t uart_rx_buffer; // Bufor na odebrany bajt z UART (Bluetooth)
const uint16_t BT_PWM_SPEED_RIGHT = 900; // Prędkość dla prawego silnika przy sterowaniu BT
const uint16_t BT_PWM_SPEED_LEFT  = 999; // Prędkość dla lewego silnika przy sterowaniu BT
/* USER CODE END PD */
...
// Wywoływana w callback do przerwania USART1
void ProcessBluetoothCommand(uint8_t command) {
    uint8_t cmd_lower = tolower(command); //normalizacja to lower case

    // Opcjonalne wysyłanie potwierdzenia odebrania wiadomości
    // snprintf(msg, sizeof(msg), "BT RX: %c\r\n", command);
    // UART_SendString(msg);

    switch (cmd_lower) {
        case 'f': // Przód
            if (current_robot_mode == ROBOT_MODE_BLUETOOTH_MANUAL) {
                robot_drive(BT_PWM_SPEED_RIGHT, 1, BT_PWM_SPEED_LEFT, 1);
        ...
        case 'b': // Tył
            if (current_robot_mode == ROBOT_MODE_BLUETOOTH_MANUAL) {
                robot_drive(BT_PWM_SPEED_RIGHT, 0, BT_PWM_SPEED_LEFT, 0);
        ...
        case 'r': // Prawo
            if (current_robot_mode == ROBOT_MODE_BLUETOOTH_MANUAL) {
                robot_drive(BT_PWM_SPEED_RIGHT, 0, BT_PWM_SPEED_LEFT, 1);
        ...
        case 'a': // Tryb autonomiczny - śledzenie linii
            ...
            line_follower_reset_state();
            UART_SendString("BT CMD: Tryb -> Śledzenie liniir");
            break;
        ...
        case 'm': // Tryb manualny - sterowanie Bluetooth
            ...
            robot_running = 0; // W trybie manualnym, `robot_running` nie kontroluje bezpośrednio
            line_follower_reset_state();
            UART_SendString("BT CMD: Tryb -> Sterowanie ręczne\r\n");
            break;

            // MIEJSCE NA KOMENDĘ DO POMIARU DYSTANSU

        default:
            UART_SendString("BT CMD: Nieznana komenda\r\n");
            break;
    }
}
/* USER CODE END 0 */
...
static void MX_TIM2_Init(void)
{
  ...
  htim2.Init.Period = 0xffffffff;					// Maksymalny okres dla 32-bitowego licznika
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    ProcessBluetoothCommand(uart_rx_buffer); // Przetwórz odebrany bajt

    // Ponownie włącz nasłuchiwanie na kolejny bajt
    HAL_UART_Receive_IT(&huart1, (uint8_t*)&uart_rx_buffer, 1);
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1)
    {
        ...
        if (HAL_UART_Receive_IT(&huart1, (uint8_t*)&uart_rx_buffer, 1) != HAL_OK)
        {
